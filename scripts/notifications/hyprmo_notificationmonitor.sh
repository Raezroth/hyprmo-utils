...
handlenewnotiffile(){
	...
	if [ "$(wc -l "$NOTIFFILE" | cut -d' ' -f1)" -lt 3 ]; then
		hyprmo_log "Invalid notification file $NOTIFFILE found (<3 lines -- see notif spec in hyprmo_notifwrite.sh), deleting!"
		rm -f "$NOTIFFILE"
	else
		hyprmo_hook_notification.sh "$NOTIFFILE" &
...
recreateexistingnotifs() {
	for NOTIF in "$HYPRMO_NOTIFDIR"/*; do
		[ -f "$NOTIF" ] || continue
...
notifications_hook() {
	hyprmo_hook_notifications.sh "$(find "$HYPRMO_NOTIFDIR"/ -type f | wc -l)"
}
...
monitorforaddordelnotifs() {
	...
	FIFO="$(mktemp -u)"
	mkfifo "$FIFO"
	inotifywait -mq -e attrib,move,delete "$HYPRMO_NOTIFDIR"  >> "$FIFO" &
	NOTIFYPID=$!
...
rm -f "$HYPRMO_NOTIFDIR"/incomingcall
recreateexistingnotifs
notifications_hook
monitorforaddordelnotifs
