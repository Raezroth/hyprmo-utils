...
checkforlostmms() {
	...
	# received
	if [ "$count" -gt 0 ]; then
		hyprmo_notify_user.sh "WARNING: Found $count unprocessed mms. Run $0 checkforlostmms --force to process them."
...
extractmmsattachement() {
	jq -r '.attrs.Attachments[] | join(",")' | while read -r aline; do
		...
		MMS_BASE_DIR="${HYPRMO_MMS_BASE_DIR:-"$HOME"/.mms/modemmanager}"
		if [ -f "$MMS_BASE_DIR/$MMS_FILE" ]; then
			OUTFILE="$MMS_FILE.$DATA_EXT"
			count=0
			while [ -f "$HYPRMO_LOGDIR/$LOGDIRNUM/attachments/$OUTFILE" ]; do
				OUTFILE="$MMS_FILE-$count.$DATA_EXT"
				count="$((count+1))"
			done
			dd skip="$AOFFSET" count="$ASIZE" \
				if="$MMS_BASE_DIR/$MMS_FILE" \
				of="$HYPRMO_LOGDIR/$LOGDIRNUM/attachments/$OUTFILE" \
				bs=1 >/dev/null 2>&1
		fi
...
processmms() {
	...
	if [ "$STATUS" = "received" ]; then
		...
		hyprmo_log "Warning: unknown status: $STATUS on $MESSAGE_PATH."
		return
	fi
	hyprmo_log "Processing $STATUS mms ($MESSAGE_PATH)."
	...
	if [ -z "$MYNUM" ]; then
		hyprmo_log "The mms file does not have a 'Modem Number'. Falling back to Me contact."
		MYNUM="$(hyprmo_contacts.sh --me)"
		if [ -z "$MYNUM" ]; then
			hyprmo_log "You do not have a Me contact. Falling back to fake number: +12345670000"
			MYNUM="+12345670000"
		fi
	fi

	SENDER="$(printf %s "$MESSAGE" | jq -r '.attrs.Sender')" # note this will be null if I am the sender
	hyprmo_debug "SENDER: $SENDER MYNUM: $MYNUM RECIPIENTS: $RECIPIENTS"
	hyprmo_debug "MESSAGE: $MESSAGE"
	...
	if cut -f1 "$SXMO_BLOCKFILE" 2>/dev/null | grep -q "^$LOGDIRNUM$"; then
		hyprmo_log "BLOCKED mms $LOGDIRNUM ($MMS_FILE)."
		HYPRMO_LOGDIR="$HYPRMO_BLOCKDIR"
	fi

	mkdir -p "$HYPRMO_LOGDIR/$LOGDIRNUM/attachments"
	printf "%s" "$MESSAGE" | extractmmsattachement

	if [ -f "$HYPRMO_LOGDIR/$LOGDIRNUM/attachments/$MMS_FILE.txt" ]; then
		TEXT="$(cat "$HYPRMO_LOGDIR/$LOGDIRNUM/attachments/$MMS_FILE.txt")"
		rm -f "$HYPRMO_LOGDIR/$LOGDIRNUM/attachments/$MMS_FILE.txt"
	else
		TEXT="<Empty>"
	fi

	dbus-send --dest=org.ofono.mms --print-reply "$MESSAGE_PATH" org.ofono.mms.Message.Delete
	hyprmo_log "Finished processing $MMS_FILE. Deleting it."

	printf "%s\t%s_mms\t%s\t%s\n" "$DATE" "$STATUS" "$LOGDIRNUM" "$MMS_FILE" >> "$HYPRMO_LOGDIR/modemlog.tsv"
	hyprmo_hook_smslog.sh "$STATUS" "$LOGDIRNUM" "$SENDER" "$DATE" "$TEXT" "$MMS_FILE" >> "$HYPRMO_LOGDIR/$LOGDIRNUM/sms.txt"

	if [ "$STATUS" = "recv" ] && [ ! "$HYPRMO_LOGDIR" = "$HYPRMO_BLOCKDIR" ]; then
		SENDER_NAME="$(hyprmo_contacts.sh --name-or-number "$SENDER")"
		...
		if [ -z "$HYPRMO_DISABLE_SMS_NOTIFS" ]; then
			...
			for attachment in "$HYPRMO_LOGDIR/$LOGDIRNUM/attachments/${MMS_FILE}".*; do
				[ -f "$attachment" ] && OPEN_ATTACHMENTS_CMD="$(printf "hyprmo_open.sh '%s'; %s" "$attachment" "$OPEN_ATTACHMENTS_CMD")"
			done
			hyprmo_log "OPEN_ATTACHMENTS_CMD: $OPEN_ATTACHMENTS_CMD"
			...
			hyprmo_notificationwrite.sh \
				...
				"$HYPRMO_LOGDIR/$LOGDIRNUM/sms.txt" \
				"$SENDER_NAME: $TEXT"
		fi
		...
		if [ "$NUM_RECIPIENTS" -gt 1 ]; then
			...
			hyprmo_hook_sms.sh "$SENDER_NAME" "$TEXT" "$MMS_FILE" "$GROUP_NAME"
		else
			hyprmo_hook_sms.sh "$SENDER_NAME" "$TEXT" "$MMS_FILE"
		fi
	fi

}

hyprmo_wakelock.sh lock hyprmo_mms_processing 30s
"$@"
hyprmo_wakelock.sh unlock hyprmo_mms_processing
