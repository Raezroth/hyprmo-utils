...
make_attachments_arg() {
	...
	done < "$HYPRMO_LOGDIR/$LOGDIRNUM/draft.attachments.txt"
}
...
if ! echo "$NUMBER" | grep -q '^+'; then
	ACTUAL_NUMBER="$(hyprmo_contacts.sh --all | grep "^$NUMBER:" | cut -d':' -f2 | sed 's/^ //')"
...
if [ "$(printf %s "$NUMBER" | xargs pnc find | wc -l)" -gt 1 ] || [ -f "$HYPRMO_LOGDIR/$NUMBER/draft.attachments.txt" ]; then
	...
	[ -f "$HYPRMO_LOGDIR/$NUMBER/draft.attachments.txt" ] && ATTACHMENTS_ARG="$(make_attachments_arg "$NUMBER" | sed 's/^ //')"
	...
	[ -f "$HYPRMO_LOGDIR/$NUMBER/draft.attachments.txt" ] && rm -f "$HYPRMO_LOGDIR/$NUMBER/draft.attachments.txt"

	# process it just like any other mms (this will handle logging it)
	hyprmo_mms.sh processmms "$MESSAGE_PATH"

	MMS_FILE="$(basename  "$MESSAGE_PATH")"
	CONTACTNAME="$(hyprmo_contacts.sh --name-or-number "$NUMBER")"
	hyprmo_hook_sendsms.sh "$CONTACTNAME" "$TEXT" "$MMS_FILE" "$CONTACTNAME"
...
# we are dealing with a normal sms, so use mmcli
	...
	SMS_RES="$(mmcli -s "${SMSNO}" --send --timeout="${HYPRMO_MM_TIMEOUT:-"30"}" 2>&1)"
	SMS_OK="$?"
	...
	hyprmo_hook_smslog.sh "sent" "$NUMBER" "$NUMBER" "$TIME" "$TEXT" >> "$HYPRMO_LOGDIR/$NUMBER/sms.txt"
	printf "%s\tsent_txt\t%s\t%s chars\n" "$TIME" "$NUMBER" "$TEXTSIZE" >> "$HYPRMO_LOGDIR/modemlog.tsv"

	CONTACTNAME="$(hyprmo_contacts.sh --name-or-number "$NUMBER")"
	hyprmo_hook_sendsms.sh "$CONTACTNAME" "$TEXT"
