...
#   Wherein $HYPRMO_CONTACTFILE is tsv with two fields: number\tcontact name
...
LOGFILE="$HYPRMO_LOGDIR"/modemlog.tsv
...
prepare_contacts_list() {
	...
	sed '/^[[:space:]]*$/d' |
	awk -F '\t' -v HYPRMO_CONTACTFILE="$HYPRMO_CONTACTFILE" '
...
[ -f "$HYPRMO_CONTACTFILE" ] || touch "$HYPRMO_CONTACTFILE"
...
if [ "$1" = "--all" ]; then
	...
	if [ -z "$2" ]; then
		...
		contact="$(hyprmo_contacts.sh --name "$2")"
		[ "$contact" = "???" ] && contact="$2"
		printf %s "$contact"
	fi
elif [ "$1" = "--name" ]; then
