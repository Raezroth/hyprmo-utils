...
if [ -n "$1" ]
then
	# E.g. passed liked: hyprmo_urlhandler.sh http://foo.com
	URL="$1"
else

	# Is normal browser? (FF or Netsurf) - use Ctrl-L Ctrl-C to copy URL
	WINDOW="$(hyprmo_wm.sh focusedwindow)"

	if printf "%s" "$WINDOW" | grep -i -E '(netsurf|firefox)'; then
		hyprmo_type.sh -M Ctrl l
		sleep 0.3
		hyprmo_type.sh -M Ctrl c
		sleep 0.3
		URL="$(hyprmo_wm.sh paste)"
	elif printf "%s" "$WINDOW" | grep -i 'geopard'; then
		hyprmo_type.sh -M Ctrl l
		sleep 0.3
		hyprmo_type.sh -M Ctrl a
		sleep 0.3
		hyprmo_type.sh -M Ctrl c
		sleep 0.3
		URL="$(hyprmo_wm.sh paste)"
