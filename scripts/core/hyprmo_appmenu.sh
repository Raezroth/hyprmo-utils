...
supertoggle_daemon() {
	if superctl status "$1" | grep -q started; then
		superctl stop "$1"
		hyprmo_notify_user.sh "$1 Stopped"
	else
		superctl start "$1"
		hyprmo_notify_user.sh "$1 Started"
	fi
...
toggle_daemon() {
	...
	if hyprmo_jobs.sh running "$1" -q; then
		hyprmo_jobs.sh stop "$@"
		notify-send "$name Stopped"
	else
		hyprmo_jobs.sh start "$@" &
		notify-send "$name Started"
	fi
}

hyprmo_type() {
	hyprmo_type.sh -s 200 "$@" # dunno why this is necessary but it sucks without
}

call_entries() {
	shown_incall_menu=
	hyprmo_modemcall.sh list_active_calls | while read -r line; do
		case "$line" in
			*"(ringing-in)")
				CALLID="$(printf %s "$line" | cut -d" " -f1 | xargs basename)"
				NUMBER="$(hyprmo_modemcall.sh vid_to_number "$CALLID")"
				CONTACT="$(hyprmo_contacts.sh --name "$NUMBER")"

				printf "%s Incoming call %s ^ 0 ^ hyprmo_jobs.sh start incall_menu hyprmo_modemcall.sh incoming_call_menu %s\n" \
					"$icon_phn" "$CONTACT" "$CALLID"
				;;
			*)
				[ -n "$shown_incall_menu" ] && continue
				shown_incall_menu=1
				printf "%s Incall Menu ^ 0 ^ hyprmo_jobs.sh start incall_menu hyprmo_modemcall.sh incall_menu\n" \
					"$icon_phn"
				;;
		esac
	done
...
getprogchoices() {
	RES="$(hyprmo_hook_contextmenu.sh "$1")"
	if [ -n "$RES" ]; then
	...
	if [ "$WINNAME" = "Sys" ]; then
		NNOTIFICATIONS="$(find "$HYPRMO_NOTIFDIR" -type f | wc -l)"
		if [ "$NNOTIFICATIONS" -gt 0 ]; then
			CHOICES="
				$icon_bel Notifications ($NNOTIFICATIONS) ^ 0 ^ hyprmo_notificationsmenu.sh
				$CHOICES
			"
		fi
	fi
	...
	echo "$WINNAME" | grep -qv Sys && CHOICES="
		$CHOICES
		$icon_mnu System Menu   ^ 0 ^ hyprmo_appmenu.sh sys
	"
...
mainloop() {
	...
	if [ -z "$CMD" ]; then
		printf "%s\n" "hyprmo_appmenu: Fallback: unknown choice <$PICKED> to contextmenu_fallback hook">&2
		sxmo_hook_contextmenu_fallback.sh "$WINNAME" "$PICKED"
		quit
	fi

	printf 'hyprmo_appmenu: Eval: <%s> from picked <%s> with loop <%s>\n' \
		"$CMD" "$PICKED" "$LOOP" >&2
