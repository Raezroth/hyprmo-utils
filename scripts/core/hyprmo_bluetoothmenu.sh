...
_full_reconnection() {
	hyprmo_terminal.sh sh -c "
notify-send 'Make the device discoverable'

bluetoothctl remove '$1'
hyprmo_jobs.sh start bluetooth_scan bluetoothctl --timeout 300 scan on
...
"
	hyprmo_jobs.sh stop bluetooth_scan
}
...
toggle_connection() {
	...
	if printf "%s\n" "$PICK" | grep -q "$icon_a2x"; then
		_can_fail timeout 7 hyprmo_terminal.sh bluetoothctl disconnect "$MAC"
	else
		_can_fail timeout 7 hyprmo_terminal.sh bluetoothctl connect "$MAC"
	fi
}
...
EOF
		...
		case "$PICK" in
			...
			"Paired $icon_tof")
				INDEX=2
				_can_fail timeout 7 hyprmo_terminal.sh bluetoothctl "$MAC"
				;;
			"Trusted $icon_ton")
				INDEX=3
				_can_fail hyprmo_terminal.sh bluetoothctl untrust "$MAC"
				;;
			"Trusted $icon_tof")
				INDEX=3
				_can_fail hyprmo_terminal.sh bluetoothctl trust "$MAC"
				;;
			"Connected $icon_ton")
				INDEX=4
				_can_fail timeout 7 hyprmo_terminal.sh bluetoothctl disconnect "$MAC"
			...
			"$icon_trh Remove")
				...
				(confirm_menu -p "Remove this device ?" \
					&& _can_fail hyprmo_terminal.sh bluetoothctl remove "$MAC") \
					|| continue
				return
				;;
		esac
...
EOF
		...
		case "$PICK" in
			...
			"Discovering $icon_tof")
				hyprmo_jobs.sh start bluetooth_scan bluetoothctl --timeout 60 scan on > /dev/null
